...
const SWATCH_DIR = path.join(process.cwd(), "public", "swatches-pro");
...
export default async function handler(
  ...
  if (fs.existsSync(swatchPath)) {
    ...
    return res.status(200).json({
      swatchUrl: `/swatches-pro/${filename}`,
  ...
  try {
    // Tier 2: DB-cached coordinates (skip Bedrock, just fetch imgix + persist)
    const cached = (await getCachedPalette(
      filename,
      "Nova Pro Pattern",
    )) as PatternResponse | null;
    ...
    console.log("[bedrock-pattern-pro] raw response:", text);
    ...
    console.log("[bedrock-pattern-pro] parsed:", parsed);
    ...
    if (
      ...
      console.error("[bedrock-pattern-pro] invalid coordinates/size:", parsed);
    ...
    await setCachedPalette(filename, "Nova Pro Pattern", parsed);
    ...
    console.error("Bedrock pattern pro error:", err);
